...
April 9: TODO list
- Add class number/class name
- use bigger images (from planktonTrain40.p)
- generate HAC Dendogram for other layers
- make it configurable for other linkage types (single, complete, ..)
'''
import numpy as np
import pickle
...
from plankton_featureMapActivations import getAverageActivationsMatrix
import matplotlib.pyplot as plt
from PIL import Image,ImageDraw
IMG_SIZE = 28
'''
This script generates a dendrogram visualization based on hierachical agglomerative clustering.
linkageType = 'single', 'complete', 'average', 'weighted', 'centroid', 'median', 'ward'
Valid methods when the raw observations are omitted are 'single', 'complete', 'weighted', and 'average'
'''
def generateDendrogramNoPic(linkageType='complete'):
    X = getAverageActivationsMatrix(which_layer=2) # default layer
    print 'Average Activations Matrix is of Shape', X.shape
    distanceMatrix = pdist(X,'euclidean')
    print 'Distance Matrix (condensed) n*(2n+1)', distanceMatrix.shape
    print 'First element', distanceMatrix[0]
    linkageMatrix = linkage(distanceMatrix, method=linkageType)
    #print 'linkage matrix',linkageMatrix
    plt.subplot(1,1,1)
    plt.title("Dendrogram - Hierachical Agglomerative Clustering of Planktons")
...
def loadListImages():
    ...
    imlist = []
    for i in range(121):
        imlist.append('results/classSamples/' + str(i) + '.png')
    return imlist

def generateDendrogram(which_layer, allFeats=False):
    print 'Generating Dendrogram for Layer', which_layer
    X = getAverageActivationsMatrix(which_layer,allFeats) # default layer
    print 'Average Activations Matrix is of Shape', X.shape
    tree = hcluster(X)
    
    filename = 'results/dendrograms/planktons_dendrogram_layer' + str(which_layer) + '.pdf'
    # list of images
    imlist = loadListImages()
    drawdendrogram(tree,imlist,jpeg=filename)
